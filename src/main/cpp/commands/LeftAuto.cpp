...
LeftAuto::LeftAuto(Drivetrain* m_swerve, Climber* m_climber, Shooter* m_shooter, Intake* m_intake, Vision* m_vision, Conveyer* m_conveyer)
:m_swerve(m_swerve),
m_climber(m_climber),
m_shooter(m_shooter),
m_intake(m_intake),
...
void LeftAuto::Execute() {
    ...
    {
        //TODO m_drive->AutoMotivateRotateLeft(); //rotate for 0.75s
    }
    else if(m_timer.Get() >= period5 && m_timer.Get() < period6) //starts at 6.25 ends at 7.55 (1.3s)
    {
        //TODO m_drive->AutoMotivateBackward(); //drive back for 1s
    }
    else
    {
        // do nothing
